...
for (const platformDir of platformDirs) {
  // Rename: opencode-darwin-arm64 → codemiecodecli-darwin-arm64
  const newName = platformDir.replace(/^opencode-/, "codemiecodecli-")
  ...
  if (fs.existsSync(binDir)) {
    ...
    const newBin = path.join(binDir, "codemiecodecli")
    if (fs.existsSync(oldBin)) {
      fs.renameSync(oldBin, newBin)
    }

    const oldExe = path.join(binDir, "opencode.exe")
    const newExe = path.join(binDir, "codemiecodecli.exe")
  ...
  if (fs.existsSync(pkgJsonPath)) {
    ...
    pkgJson.name = pkgJson.name.replace(/^opencode-/, "codemiecodecli-")
    fs.writeFileSync(pkgJsonPath, JSON.stringify(pkgJson, null, 2))
  }
}

console.log("codemiecodecli build complete!")
